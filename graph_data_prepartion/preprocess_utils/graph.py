...
def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part1(data):
    ...
    extra_nodes = set()
    for qid in qa_nodes:
        if qid in cpnet_simple.nodes:
            extra_nodes |= set(cpnet_simple[qid])
    extra_nodes = extra_nodes - qa_nodes
    return (sorted(qc_ids), question, sorted(extra_nodes))

def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part2(data):
    qc_ids, question, extra_nodes = data

    cid2score = get_LM_score(qc_ids+extra_nodes, question)
    return (qc_ids, question, extra_nodes, cid2score)

def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part3(data):
    qc_ids, question, extra_nodes, cid2score = data

    schema_graph = qc_ids + sorted(extra_nodes, key=lambda x: -cid2score[x]) #score: from high to low
    arange = np.arange(len(schema_graph))
    qmask = arange < len(qc_ids)
    amask = (arange >= len(qc_ids)) & (arange < (len(qc_ids)))
    adj, concepts = concepts2adj(schema_graph)
...
def generate_adj_data_from_grounded_concepts__use_LM(grounded_path, cpnet_graph_path, cpnet_vocab_path, output_path, num_processes):
    ...
    with open(grounded_path, 'r', encoding='utf-8') as fin_ground, open(statement_path, 'r', encoding='utf-8') as fin_state:
        ...
        for j, line in enumerate(lines_ground):
            ...
            statement_obj = json.loads(lines_state[dic['doc_id']])
            QAcontext = "{}.".format(statement_obj['statements'][j])
    ...
    # for q in qa_data:
    #     res1.append(concepts_to_adj_matrices_2hop_all_pair__use_LM__Part1(q))

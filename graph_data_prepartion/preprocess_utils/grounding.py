...
def ground_qa_pair(sent):
    ...
    return {"sent": s, "qc": all_concepts, "ac": None, "doc_id": doc_id}
...
def match_mentioned_concepts(sents, num_processes):
    with Pool(num_processes) as p:
        res = list(tqdm(p.imap(ground_qa_pair, sents), total=len(sents)))
    return res
...
def ground(statement_path, cpnet_vocab_path, pattern_path, output_path, num_processes=20, debug=False):

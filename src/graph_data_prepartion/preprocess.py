    ...
    output_paths = {
        ...
        'gov-reports': {
            'statement': {
                ...
                'val': f'{BASE_DIR}/{DS_DIR}/statement/val.statement.part.{SERVER_NAME}.jsonl',
                'test': f'{BASE_DIR}/{DS_DIR}/statement/test.statement.part.{SERVER_NAME}.jsonl',
            },
            'ground': {
                # 'train': f'{BASE_DIR}/{DS_DIR}/ground/train.ground.part.{SERVER_NAME}.jsonl',
                'train': f'{BASE_DIR}/{DS_DIR}/ground/train.ground.part.{SERVER_NAME}.jsonl',
                'val': f'{BASE_DIR}/{DS_DIR}/ground/val.ground.part.{SERVER_NAME}.jsonl',
            ...
            'graph': {
                ...
                'adj-train': f'{BASE_DIR}/{DS_DIR}/graph/train.graph.part.{SERVER_NAME}.adj.pk',
                'adj-val': f'{BASE_DIR}/{DS_DIR}/graph/val.graph.part.{SERVER_NAME}.adj.pk',
...
def main():
    ...
    parser.add_argument('--distributed_processing', type=bool, default=False, help='enable debug mode')
    ...
    if K!=-1 and SEED!=-1:
        # append SEED and K to the end of input and output files...
        for key in ['statement', 'ground', 'graph']:
            new_input = {}
            for se, path in output_paths['gov-reports'][key].items():
                if se == 'test':
                    new_input[se] = path

                if key=='graph':
                    new_input[se] = path.replace('.pk', '') + f'.seed{SEED}.k{K}.pkl'
                else:
                    new_input[se] = path.replace('.json', '') + f'.seed{SEED}.k{K}.json'

            output_paths['gov-reports'][key] = new_input
    ...
    if not args.only_test:
        routines = {
            ...
            'cnndm': [
                ...
                {'func': convert_to_entailment, 'args': (input_paths['gov-reports']['train'], output_paths['gov-reports']['statement']['train'], SEED, K, args.distributed_processing)},
                {'func': convert_to_entailment, 'args': (input_paths['gov-reports']['val'], output_paths['gov-reports']['statement']['val'], SEED, K, args.distributed_processing)},
                # {'func': convert_to_entailment, 'args': (input_paths['gov-reports']['test'], output_paths['gov-reports']['statement']['test'], SEED, K, args.distributed_processing, True)},
